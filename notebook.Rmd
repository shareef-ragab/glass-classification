...
```{r}
mlrcontrol <- trainControl(method = "cv", number = 10)
mlrcontrol

```
```{r}
trainRI <- train(RIcluster~., trainDate, method = "glmnet", 
             metric = "Accuracy", 
             tuneGrid = expand.grid(alpha = 0.1, lambda = 0.004972569),
             trControl = mlrcontrol)

PredictRIClusetr <- predict(trainRI, testDate)
confusionMatrix(testDate$RIcluster, PredictRIClusetr)
```
```{r}
trainRI <- train(RIcluster~., trainDate, 
             method = "svmRadial", 
