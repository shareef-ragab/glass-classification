...
ggplot(date, aes(x=Na, fill=RIcluster)) + geom_density()
```
```{r}
png("Mg_RIcluster.png");
ggplot(date, aes(x=Mg, fill=RIcluster)) + geom_density()
```

```{r}
png("AL_RIcluster.png");
ggplot(date, aes(x=Al, fill=RIcluster)) + geom_density()
```

```{r}
png("Si_RIcluster.png");
ggplot(date, aes(x=Si, fill=RIcluster)) + geom_density()
```

```{r}
png("K_RIcluster.png");
ggplot(date, aes(x=K, fill=RIcluster)) + geom_density()
```

```{r}
png("Ca_RIcluster.png");
ggplot(date, aes(x=Ca, fill=RIcluster)) + geom_density()
```
